<?php
	...
	if(strlen($_POST['password'])<6 || 
		...
		js_redirect_post("signup.php", "Your password can only contain letters, numbers, and an underscore, must be at least 6 characters, and contain at least one lowercase letter, one uppercase letter, and one number!", array("username" => $userSan, "email" => $emailSan));
		exit();
	 }
	 ...
	 if(strlen($user)<6 || 
		...
		js_redirect_post("signup.php", "Your username can only contain letters, numbers, and an underscore and must be between 6 and 32 characters!", array("username" => $userSan, "email" => $emailSan));
		exit();
	 }

	 // check if email is valid
	 if(!filter_var($email, FILTER_VALIDATE_EMAIL)){
		js_redirect_post("signup.php", "The given email address is not valid!", array("username" => $userSan, "email" => $emailSan));
		exit();
	 }
	 ...
	 if($userStatement->fetchAll()){
		js_redirect_post("signup.php", "That username is already in use!", array("username" => $userSan, "email" => $emailSan));
		exit();
	 }
	 else{
		...
		if($res = $emailStatement->fetchAll()){
			js_redirect_post("signup.php", "That email is already in use!", array("username" => $userSan, "email" => $emailSan));
			exit();
		}
		else {
